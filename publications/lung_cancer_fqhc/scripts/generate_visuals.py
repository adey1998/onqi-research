...
def save_figure(fig, name):
    """Save figure in SVG, PDF, and PNG formats (publication-ready)"""
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    for ext in ["svg", "pdf", "png"]:  # Added PNG export
        if ext == "png":
            fig.savefig(os.path.join(OUTPUT_DIR, f"{name}.{ext}"), dpi=300, bbox_inches="tight")  # High-res PNG
        else:
...
def generate_visuals():
    ...
    save_figure(fig, "fig1_eligibility_breakdown")
    ...
    plt.tight_layout()
    save_figure(fig, "fig2_ineligibility_reasons")
    ...
    ax.legend()
    save_figure(fig, "fig3_pack_years_distribution")
    ...
    ax.grid(axis="y", linestyle="--", alpha=0.7)
    save_figure(fig, "fig4_quit_years_distribution")
    ...
    save_figure(fig, "fig5_age_vs_eligibility")
    ...
    save_figure(fig, "fig6_missed_high_risk_smokers")
    ...
    ax.grid(axis="y", linestyle="--", alpha=0.7)
    save_figure(fig, "fig7_referral_workflow_funnel")
